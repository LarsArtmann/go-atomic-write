...
| Documentation site (9 pages)        | 🟢 `FULLY_FUNCTIONAL`     | `website/src/content/docs/`; all pages teach the current `v0.4.0 API split` (`Write`/`WriteVerified`/`WriteIfChanged`/`WriteFunc`/`WriteFuncVerified`). Verified by `npm run build` + `typecheck` (11 pages, 0 errors). Changelog page is **generated** from root `CHANGELOG.md`. |
